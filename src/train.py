...
class Transform(object):
    ...
    def __call__(self, in_data):
        ...
        bboxes = transforms.translate_bbox(
            bboxes, y_offset=param['y_offset'], x_offset=param['x_offset'])

        # 3. Random cropping
        img, param = random_crop_with_bbox_constraints(
            img, bboxes, return_param=True)
        bboxes, param = transforms.crop_bbox(
            bboxes, y_slice=param['y_slice'], x_slice=param['x_slice'],
            allow_outside_center=False, return_param=True)
        labels = labels[param['index']]
        ...
        bboxes = transforms.flip_bbox(
            bboxes, (self.size, self.size), x_flip=params['x_flip'])

        # Preparation for SSD network
        img -= self.mean
        mb_loc, mb_label = self.coder.encode(bboxes, labels)
